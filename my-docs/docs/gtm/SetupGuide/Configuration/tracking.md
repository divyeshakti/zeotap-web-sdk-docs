...
*   **Blacklisted Fields**: Certain fields, such as `country` and `city`, are often automatically blacklisted for ingestion by Zeotap's systems because this information is typically derived with higher accuracy from the user's IP address on the server-side. 
    *   For a complete and up-to-date list of fields that are automatically blacklisted by Zeotap, please refer to the official Zeotap documentation or contact your Zeotap representative. ([Blacklisted fields](https://docs.zeotap.com/articles/#!integrate-customer/blacklisted-fields-for-data-ingestion)).
