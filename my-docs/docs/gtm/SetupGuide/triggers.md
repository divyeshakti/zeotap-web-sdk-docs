...
Once the inputs for the Zeotap Collect Tag have been configured (see [Step 3: Configure the Tag](/gtm/docs/SetupGuide/Configuration/configurations)), the next step is to define **when** the tag should fire. This is done by setting up triggers within Google Tag Manager (GTM).
